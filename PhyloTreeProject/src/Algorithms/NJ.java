...
public class NJ extends Method {
    ...
    public NJ(ArrayList<String> sequences) {
    	super(sequences);
    	...
    	while(!isMatrixFinished(DistanceMatrix)) { 
	    	...
	    	double[] lengths = branchLengths(numSeq, DistanceMatrix, index1, index2, Sums);
	    	...
	    	for(int i=0; i<tree.getNodes().length; i++) {
	    		System.out.println(tree.getNodes()[i]);
	    	}
	    	System.out.println();
    	}
    	...
    	for (int i = 0; i<Qmatrix.size(); i++) {
    		for (int j = 0; j<Qmatrix.get(i).size(); j++) {
    			if(Qmatrix.get(i).get(j) == 0.0){
    				...
    				double newDistance = Qmatrix.get(i).get(j) - (Sums[i] + Sums[j])/(numSeq - 2);
    ...
    public double[] branchLengths(double numSeq, ArrayList<ArrayList<Double>> Matrix, int cluster1, int cluster2, double[] Sums){
    	double[] lengths = null;
    ...
    public void mergeClusters(double[] clusterSize, int cluster1, int cluster2, double[] branchLengths, NJTree tree, int maxNodes) {
    	if(clusterSize[cluster1] == 1.0 && clusterSize[cluster2] == 1.0) {
			...
			if(tree.getMainNode().getNumberOfNodes() < maxNodes) {
				...
				if(tree.getNodes()[cluster1].getLength() == 0.0) {
					tree.getNodes()[cluster1].setLength(branchLengths[0]);
				}
			}
			if(SequenceList[cluster2].getLength() == 0.0) {
				SequenceList[cluster2].addLength(branchLengths[1]);
			}
		}
		else if(clusterSize[cluster1] == 1.0 && clusterSize[cluster2] > 1.0) {
			if(tree.getMainNode().getNumberOfNodes() < maxNodes) {
				...
				if(tree.getNodes()[cluster2].getLength() == 0.0) {
					tree.getNodes()[cluster2].setLength(branchLengths[1]);
					tree.updateNodes(cluster1, cluster2);
				}
			}
			if(SequenceList[cluster1].getLength() == 0.0) {
				SequenceList[cluster1].addLength(branchLengths[0]);
			}
		}
		else {
			if(tree.getMainNode().getNumberOfNodes() < maxNodes) {
				tree.merge(tree.getNodes()[cluster1], tree.getNodes()[cluster2], cluster1, branchLengths[0]);
			}
			else {
				if(tree.getNodes()[cluster1] == null) {
					tree.getNodes()[cluster2].setLength(branchLengths[0]);
				}
				else if(tree.getNodes()[cluster1].getLength() == 0.0) {
					tree.getNodes()[cluster1].setLength(branchLengths[0]);
				}
			}
			if(tree.getNodes()[cluster2].getLength() == 0.0) {
