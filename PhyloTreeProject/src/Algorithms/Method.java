...
public class Method {
    ...
    public Method(ArrayList<String> sequences) {
        NumSeq = sequences.size();
        DistanceMatrix = new ArrayList<>();
        SequenceList = new Sequence[NumSeq];
        for (int i = 0; i < NumSeq; i++) {
        	//create branch for each sequence
            SequenceList[i] = new Sequence(sequences.get(i), SelectFiles.getSampleNames()[i]);
            System.out.println(SequenceList[i].getSeq());
	    ...
	    for (int i = 0; i < NumSeq; i++) {
	        for (int j = 0; j < NumSeq; j++) {
	        	String sequence1 = sequences.get(i).replaceAll("[\n]", ""); //take out newline caracters
	        	String sequence2 = sequences.get(j).replaceAll("[\n]", "");
